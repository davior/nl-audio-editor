...
| Operation | Status | Notes |
|---|---|---|
| `line_reduce` | **built** | Static, zero-phase. `lines` `auto` or explicit `[{freq_hz, width_hz, depth_db}]`; `min_prominence_db` 1–40 (6); `min_persistence` 0–1 (0.5); `max_lines` 1–64 (16); `target_excess_db` 0–24 (0 = match the surroundings); `max_depth_db` 0–60 (40); `width_factor` 0.5–4 (1). Detection band from a `band` or `tf_patch` scope. Cuts each line by its measured prominence, re-measures, and cuts again until nothing stands out; records the largest remaining prominence. |
